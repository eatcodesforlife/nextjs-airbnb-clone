...
import { SearchIcon, MenuIcon, UserCircleIcon, GlobeAltIcon, ChevronLeftIcon } from '@heroicons/react/solid'
...
const Header = ({placeholder}) => {
  ...
  return (
    <div className='sticky top-0 z-50 grid grid-cols-3 p-5 bg-white shadow-md md:px-10'>
      ...
      <div 
        onClick={() => router.push('/')}
        className="relative hidden md:flex items-center h-9 cursor-pointer my-auto">
      ...
      <div
        ...
        { router.pathname != '/' && <ChevronLeftIcon  
          onClick={() => router.push('/')} 
          className='inline-flex  h-8 text-gray-600 rounded-full cursor-pointer'  />
        }
      </div>  
      {/* search */}
